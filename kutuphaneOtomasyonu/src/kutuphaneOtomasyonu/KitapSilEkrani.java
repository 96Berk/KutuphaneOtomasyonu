	...
	public KitapSilEkrani() {
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				sql="DELETE FROM kitaplar WHERE kitap_adi = '"+secilenKitap+"'";
				
				try {
				VeriTabaniBaglantisi.delete(sql);
				JOptionPane.showMessageDialog(contentPane, "Kitap Silindi!");
				kitaplariYukle();
				}
				catch(Exception e3){
					e3.printStackTrace();
					JOptionPane.showMessageDialog(contentPane, "Kitap Silinmedi!", "Hata3", JOptionPane.ERROR_MESSAGE);
				}
